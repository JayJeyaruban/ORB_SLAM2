...
namespace ORB_SLAM2 {
    ...
    thread_local long unsigned int KeyFrame::nNextId = 0;
    ...
    KeyFrame::KeyFrame(KeyFrame &KF, Map *pMap) :
            ...
            cy(KF.cy), invfx(KF.invfx), invfy(KF.invfy), mbf(KF.mbf), mb(KF.mb), mThDepth(KF.mThDepth), N(KF.N),
            mvKeys(KF.mvKeys), mvKeysUn(KF.mvKeysUn), mvuRight(KF.mvuRight), mvDepth(KF.mvDepth),
            mDescriptors(KF.mDescriptors.clone()), mBowVec(KF.mBowVec), mFeatVec(KF.mFeatVec),
            mnScaleLevels(KF.mnScaleLevels), mfScaleFactor(KF.mfScaleFactor), mfLogScaleFactor(KF.mfLogScaleFactor),
            mvScaleFactors(KF.mvScaleFactors), mvLevelSigma2(KF.mvLevelSigma2), mvInvLevelSigma2(KF.mvInvLevelSigma2),
            mnMinX(KF.mnMinX), mnMinY(KF.mnMinY), mnMaxX(KF.mnMaxX), mnMaxY(KF.mnMaxY), mK(KF.mK),
            mvpMapPoints(KF.mvpMapPoints), mpKeyFrameDB(KF.mpKeyFrameDB), mpORBvocabulary(KF.mpORBvocabulary),
            mbFirstConnection(true), mpParent(NULL), mbNotErase(false), mbToBeErased(false), mbBad(false),
            mHalfBaseline(KF.mb / 2), mpMap((pMap != NULL) ? pMap : KF.mpMap) {
        mnId = nNextId++;
